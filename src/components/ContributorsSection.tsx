...
const ContributorsSection: React.FC = () => {
    ...
    const fetchContributors = async () => {
      try {
        const response = await fetch(
          `https://api.github.com/repos/${repoUrl}/contributors`
        );
  ...
  useEffect(() => {
    ...
    if (!isInteracting && !isLoading && !error) {
          ...
          {isLoading ? (
            <div className="flex overflow-x-hidden py-8 md:py-12 space-x-6 mx-auto justify-center">
              {[...Array(4)].map((_, i) => (
                <div
                  key={i}
                  className="flex flex-col w-44 md:w-52 lg:w-60 shrink-0"
                >
                  <div
            ...
            >
              <div className="flex items-end mx-auto gap-4 md:gap-8 hover:gap-8">
                {contributors.map((person, index) => (
                  <div
                    key={`${person.id}-${index}`}
                    className={`group shrink-0 transition-all duration-500 contributor-float flex ${
                      isVisible
